...
int main()
{
	...
	switch (option)
	{
	    ...
	    case 2:
	    	...
	    	if ((ret = sl_insert_after(&head, gdata, data)) == DATA_NOT_FOUND) //pass by reference
	    	{
	   ...
	   case 3:
    		...
    		scanf("%d", &data);
	    	/*insert_at_last function call*/
    		if ((ret = sl_insert_before(&head, gdata, data)) == DATA_NOT_FOUND) //pass by reference
	    	{
	    	    printf("INFO %d is not found at the list\n", gdata);
    		}
	    	else if (ret == LIST_EMPTY)
	    ...
	    case 4:
		    ...
		    scanf("%d", &data);
		    /*insert_at_last function call*/
		    if ((ret = sl_insert_nth(&head, pos, data)) == POSITION_NOT_FOUND)//pass by reference
		    {
	    ...
	    case 5:
		    ...
		    scanf("%d", &data);
		    if ((ret = sl_delete_element(&head, data)) == DATA_NOT_FOUND)
		    {
